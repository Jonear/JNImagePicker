...
NSInteger PageInBound(NSInteger value, NSInteger min, NSInteger max)
{
    if (max < min) {
        max = min;
    }
...
- (void)loadPagesForCurrentPage:(NSInteger)currentPage
{
    ...
    NSInteger first = PageInBound(currentPage - 1, 0, count - 1);
    NSInteger last  = PageInBound(currentPage + 1, 0, count - 1);
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        else
        {
            ...
            if ([obj isKindOfClass:[UIView class]])
            {
                if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(pageView:didEndDisplayingAtIndex:)]) {
                    [_pageViewDelegate pageView:self didEndDisplayingAtIndex:index];
                }
